BEGIN;
ALTER TABLE public.new_family DROP CONSTRAINT new_family_education_progress_check;
ALTER TABLE public.new_family ADD CONSTRAINT new_family_education_progress_check CHECK (education_progress BETWEEN 0 AND 4);
-- 직접 선택한 진도와 기존 교육 차수별 기록을 구분하여 과거 기록을 보존한다.
CREATE OR REPLACE FUNCTION public.new_family_set_simple_education(p_family_id integer, p_progress integer)
...
BEGIN
  ...
  IF p_progress IS NULL OR p_progress NOT BETWEEN 0 AND 4 THEN RAISE EXCEPTION '교육 주차를 확인해주세요.'; END IF;
  ...
  UPDATE public.new_family SET education_progress=p_progress, education_updated_by=auth.uid(), step=CASE WHEN p_progress=0 THEN 1 WHEN p_progress=4 THEN 3 ELSE 2 END, step_updated_at=now() WHERE id=f.id;
END; $$;
